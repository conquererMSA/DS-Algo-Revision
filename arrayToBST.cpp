...
/*
recursive function e node gulu nich theke connect hoye uporer dike uthte thake... protome null node gulu tar parent er sathe connect hoy, evabe node gulu tar parent er sathe connect hote thake...
*/
BTreeNode* sortedArrayToBST(int leftIndex, int rightIndex, int sortedArray[]){
    if(leftIndex>rightIndex) return NULL;
    int midIndex=(leftIndex+rightIndex)/2;
    BTreeNode* root=new BTreeNode(sortedArray[midIndex]);
    BTreeNode* leftNode=sortedArrayToBST(leftIndex, midIndex-1, sortedArray);
    BTreeNode* rightNode=sortedArrayToBST(midIndex+1,rightIndex,sortedArray);
    root->left=leftNode;
...
int main() {
    ...
    int leftIndex=0;
    int rightIndex=n-1;
    BTreeNode* root=sortedArrayToBST(leftIndex, rightIndex, sortedArray);
