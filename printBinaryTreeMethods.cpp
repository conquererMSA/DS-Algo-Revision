...
void preOrderTraversalOfBTree(BTreeNode* root){
    if(root==NULL) return;
    cout<<root->val<<" ";
    preOrderTraversalOfBTree(root->left);
    preOrderTraversalOfBTree(root->right);
}
void postOrderTraversalOfBTree(BTreeNode* root){
    if(root==NULL) return;
    postOrderTraversalOfBTree(root->left);
    postOrderTraversalOfBTree(root->right);
    cout<<root->val<<" ";
}
void inOrderTraversalOfBTree(BTreeNode* root){
    if(root==NULL) return;
    inOrderTraversalOfBTree(root->left);
    cout<<root->val<<" ";
    inOrderTraversalOfBTree(root->right);
    
}
int main() {
    BTreeNode* root = levelOrderTreeInput();
    preOrderTraversalOfBTree(root); //ascending
    cout<<endl;
    postOrderTraversalOfBTree(root);
    cout<<endl;
    inOrderTraversalOfBTree(root);
